module nebula::user{
    ...
    // ======== PUBLIC FUNCTIONS =========
    public fun new_user(username: String, ctx: &mut TxContext): User {
        User{
            id: object::new(ctx),
            username,
            betWin: 0,
    ...
    // ======= GETTERS =========
    public fun get_user_stats(user: &User): (String, u64, u64, String, u64){
        (user.username, user.betWin, user.betLost, user.grade, balance::value(&user.balance))
    }
}